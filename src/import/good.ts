...
const STAT_MAP: Record<string, StatKey> = {
  ...
  eleMas: 'em', enerRech_: 'er_pct', critRate_: 'crit_rate', critDMG_: 'crit_dmg',
  pyro_dmg_: 'elemental_dmg', hydro_dmg_: 'elemental_dmg', electro_dmg_: 'elemental_dmg',
  cryo_dmg_: 'elemental_dmg', anemo_dmg_: 'elemental_dmg', geo_dmg_: 'elemental_dmg',
  dendro_dmg_: 'elemental_dmg', physical_dmg_: 'physical_dmg', heal_: 'healing',
};

interface GoodArtifact {
  setKey: string; slotKey: string; rarity: number; level: number;
  mainStatKey: string; substats: { key: string; value: number }[];
}
...
export function parseGOOD(json: unknown): Artifact[] | { error: 'BAD_FORMAT' } {
  ...
  if (obj.format !== 'GOOD' || !Array.isArray(obj.artifacts)) return { error: 'BAD_FORMAT' };
  ...
  for (const raw of obj.artifacts as GoodArtifact[]) {
    ...
    out.push({
      id: crypto.randomUUID(), setKey: raw.setKey, slot, rarity: raw.rarity, level: raw.level,
      mainStat, mainStatValue: genshinAdapter.mainStatValue(mainStat, raw.rarity, raw.level), subStats,
    });
  }
  return out;
}
