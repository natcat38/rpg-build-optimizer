...
const PROP_STAT: Record<string, StatKey> = {
  FIGHT_PROP_HP: 'hp', FIGHT_PROP_HP_PERCENT: 'hp_pct', FIGHT_PROP_ATTACK: 'atk',
  FIGHT_PROP_ATTACK_PERCENT: 'atk_pct', FIGHT_PROP_DEFENSE: 'def', FIGHT_PROP_DEFENSE_PERCENT: 'def_pct',
  FIGHT_PROP_ELEMENT_MASTERY: 'em', FIGHT_PROP_CHARGE_EFFICIENCY: 'er_pct',
  FIGHT_PROP_CRITICAL: 'crit_rate', FIGHT_PROP_CRITICAL_HURT: 'crit_dmg',
  FIGHT_PROP_HEAL_ADD: 'healing', FIGHT_PROP_PHYSICAL_ADD_HURT: 'physical_dmg',
  FIGHT_PROP_FIRE_ADD_HURT: 'elemental_dmg', FIGHT_PROP_WATER_ADD_HURT: 'elemental_dmg',
  FIGHT_PROP_ELEC_ADD_HURT: 'elemental_dmg', FIGHT_PROP_ICE_ADD_HURT: 'elemental_dmg',
  FIGHT_PROP_WIND_ADD_HURT: 'elemental_dmg', FIGHT_PROP_ROCK_ADD_HURT: 'elemental_dmg',
  FIGHT_PROP_GRASS_ADD_HURT: 'elemental_dmg',
};

export async function fetchUidArtifacts(uid: string): Promise<Artifact[] | UidError> {
  let res: Response;
  try {
    res = await fetch(`https://enka.network/api/uid/${encodeURIComponent(uid)}`);
  } catch {
  ...
  if (!Array.isArray(avatars) || avatars.length === 0) return { error: 'NO_SHOWCASE' };

  const out: Artifact[] = [];
  for (const av of avatars as Array<{ equipList?: Array<{ reliquary?: { level?: number }; flat?: Record<string, unknown> }> }>) {
    for (const equip of av.equipList ?? []) {
      ...
      const reliquaryMainstat = flat.reliquaryMainstat as { mainPropId?: string; statValue?: number } | undefined;
      const mainStat = PROP_STAT[reliquaryMainstat?.mainPropId ?? ''];
      if (!slot || !mainStat) continue;
      const rawSubs = flat.reliquarySubstats as Array<{ appendPropId: string; statValue: number }> | undefined;
