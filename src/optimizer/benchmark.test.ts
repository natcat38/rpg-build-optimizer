...
import { makeInventory, DEFAULT_SEED } from './benchmark';
...
describe('search bound regression', () => {
  // docs/speed-report.md is regenerated by hand (`npm run bench`), so a bound
  // that quietly loosens would only show up as a stale report nobody re-ran.
  // Pin one seeded scenario here instead: if `explored` climbs, CI fails.
  it('explores no more leaves than the pinned ceiling on a 50-piece inventory', () => {
    const inv = makeInventory(50, DEFAULT_SEED);
    ...
    const r = searchBuilds(req, inv, buildContext(req));
    expect(r.status).toBe('ok');
    // Observed: 270 leaves out of 10^5 = 100,000 naive builds (the previous
    // k*6-retention bound explored 1,080). The ceiling has slack for dataset
    // drift on a patch refresh, but not for a bound regression. If a data
    // refresh legitimately moves this, re-run `npm run bench` and re-pin.
    expect(r.explored).toBeGreaterThan(0);
    expect(r.explored).toBeLessThanOrEqual(400);
  });
});
