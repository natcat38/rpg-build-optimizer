...
**D2. Imaginarium Theater + Stygian Onslaught modes.** Blocked on D1-adjacent curation, not on code: each mode needs per-patch modifiers (Theater's element restrictions, Stygian's boss lineup, Abyss blessings) modelled as data plus mode-specific recommenders next to `recommendAbyss`. Prerequisite shipped in Task 12 (runbook + patch visibility). The spec should decide how much per-patch curation the maintainer is actually willing to do monthly — that constraint, not engineering, sizes this feature.
