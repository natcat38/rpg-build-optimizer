...
**Scope:** v1.0 only. Gap analysis, the meta snapshot, the "+20 projection" toggle, the live search animation, E2E robot, and the second-game stub are **out of scope** (v1.1 — see `docs/superpowers/specs/2026-06-05-depth-layer-and-portfolio-design.md`). The `GameAdapter` interface is built now; only the Genshin adapter is implemented.
...
const snapshot = {
  patch: PATCH,
  characters: [] as Array<{ key: string; name: string; element: string; baseByLevel: Record<string, Record<string, number>> }>,
  weapons: [] as Array<{ key: string; name: string; type: string; byLevel: Record<string, Record<string, number>> }>,
  sets: [] as Array<{ key: string; name: string; twoPiece?: Record<string, number>; fourPiece?: Record<string, number> }>,
...
console.log(`Wrote snapshot for patch ${PATCH}: ${snapshot.characters.length} characters, ${snapshot.weapons.length} weapons, ${snapshot.sets.length} sets.`);
```
...
const STAT_KEYS: StatKey[] = [
  'hp', 'hp_pct', 'atk', 'atk_pct', 'def', 'def_pct', 'em', 'er_pct',
  'crit_rate', 'crit_dmg', 'elemental_dmg', 'physical_dmg', 'healing',
];
...
export const genshinAdapter: GameAdapter = {
  ...
  characters(): CharacterMeta[] {
    return data.characters.map((c) => ({ key: c.key, name: c.name, element: c.element as CharacterMeta['element'] }));
  },

  weapons(): WeaponMeta[] {
    return data.weapons.map((w) => ({ key: w.key, name: w.name, type: w.type }));
  },

  sets(): ArtifactSetMeta[] {
    return data.sets.map((s) => ({ key: s.key, name: s.name, twoPiece: vec(s.twoPiece), fourPiece: s.fourPiece ? vec(s.fourPiece) : undefined }));
  },
  ...
  baseStats(characterKey: string, weaponKey: string, level: BuildLevel): StatVec {
    ...
    const add = (src?: Record<string, number>) => {
      if (!src) return;
      for (const k of Object.keys(src)) out[k as StatKey] = (out[k as StatKey] ?? 0) + src[k];
    };
  ...
  return (
    <div className="space-y-2 p-4 border rounded">
      ...
      <label className="block">Level
        <input className="ml-2 border w-16" type="number" value={level} onChange={(e) => setLevel(Number(e.target.value))} />
      </label>
  ...
  for (const av of avatars) {
    for (const equip of av.equipList ?? []) {
      ...
      const subStats: SubStat[] = (flat.reliquarySubstats ?? [])
        .map((s: { appendPropId: string; statValue: number }) => ({ key: PROP_STAT[s.appendPropId], value: s.statValue }))
      ...
      out.push({
        id: crypto.randomUUID(), setKey: String(flat.setNameTextMapHash), slot, rarity, level,
        mainStat, mainStatValue: genshinAdapter.mainStatValue(mainStat, rarity, level), subStats,
      });
    }
  }
  return out;
...
> Note on `setKey`: Enka exposes a set name *hash*, not the genshin-db set key. For v1.0, set-requirement constraints work most reliably with GOOD imports. Document this limitation in the ImportPanel; a hash→key mapping table is a v1.1 refinement.
...
```ts
import type { Artifact, OptimizeContext, OptimizeRequest, OptimizeResult, BuildResult, Slot, StatVec } from '../game/types';
import { SLOTS } from '../game/types';
import { totals, objectiveValue, satisfies, critRatioPenalty, addInto, countSets } from './score';
...
interface Heap { items: BuildResult[]; }
...
export function optimize(req: OptimizeRequest, inventory: Artifact[], ctx: OptimizeContext): OptimizeResult {
  ...
  function minScoreInHeap(): number {
    return heap.items.length >= k ? heap.items[Math.min(k, heap.items.length) - 1].score : -Infinity;
  }
  ...
  function recurse(slotIndex: number, runningObjective: number) {
    ...
    if (upper <= minScoreInHeap()) { pruned++; return; }
  ...
  if (heap.items.length === 0) return { builds: [], explored, pruned, reason: 'NO_FEASIBLE_BUILD' };
...
import type { BuildLevel, Objective, OptimizeRequest, Slot, StatKey } from '../game/types';
...
export function BuildCard({ build, request, onShare }: { build: BuildResult; request: OptimizeRequest; onShare?: () => void }) {
  return (
    <div className="border rounded p-3 space-y-2">
      ...
      <ul className="text-sm grid grid-cols-2 gap-x-4">
        {SHOW.map((k) => <li key={k} className="flex justify-between"><span>{k}</span><span>{(build.totals[k] ?? 0).toFixed(1)}</span></li>)}
      </ul>
      <div className="text-xs text-gray-500">
        Pieces: {SLOTS.map((s) => `${s}:${build.artifactIds[s]?.slice(0, 6) ?? '—'}`).join('  ')}
      </div>
    </div>
  );
}
...
export function Results({ result, request }: { result: OptimizeResult; request: OptimizeRequest }) {
  const [copied, setCopied] = useState<number | null>(null);
  if (result.reason === 'NO_FEASIBLE_BUILD' || result.builds.length === 0) {
    return <p className="p-4 text-amber-700">No build satisfies all constraints. Try relaxing the set requirement or the Energy Recharge minimum.</p>;
  }
  return (
    <div className="space-y-3">
      <p className="text-xs text-gray-500">Explored {result.explored} combinations, pruned {result.pruned}.</p>
      {result.builds.map((b, i) => (
        <BuildCard key={i} build={b} request={request} onShare={async () => {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <section>
        ...
        <OptimizePanel onResult={(r, req) => { setResult(r); setRequest(req); }} />
      </section>
...
- **Spec coverage:** Import (GOOD+UID) → Tasks 5–6; manual entry + validation → Task 4; constraints/target UI → Task 10; exact optimiser + diagnostics + anti-clone → Tasks 7–8; worker → Task 9; results + compare entry → Task 11; share encode/decode + hydration → Tasks 12–13; dataset + adapter + build level → Tasks 2–3; shell/states/deploy → Tasks 1, 13. Set-requirement & main-stat-lock *engine* support is in Tasks 7–8; their *UI inputs* are flagged as an in-task follow-up in Task 10 (the request shape already carries them).
