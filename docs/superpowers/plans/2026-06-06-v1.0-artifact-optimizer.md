...
**Scope:** v1.0 only. Gap analysis, the meta snapshot, the "+20 projection" toggle, the live search animation, E2E robot, and the second-game stub are **out of scope** (v1.1 — design doc was never written). The `GameAdapter` interface is built now; only the Genshin adapter is implemented.
